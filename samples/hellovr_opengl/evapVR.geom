...
vec2 findNewST(float tS, float tT){
 //distance between 0-1 and 0 - 2
	st01 *= vec2(tS, 0);
	st02 *= vec2(0, tT);
	return v2UVcoords[0] + st01 + st02;
}


// float getNoise(vec2 vert){
// 	//vec4 nv  = texture2D( Noise2, 10.*vert );
...
float getNoise3D(vec3 vert){
	vec4 nv  = texture3D( Noise2, 10*vert );
    float n = nv.r + nv.g + nv.b + nv.a;    //  1. -> 3.
    n = n - 2.;                             // -1. -> 1.
...
//if t <=  uTimer * 10 /8 fly
//then tVel = min((uTimer* 10 /8 - t) * 2,1);
void
...
main( )
{
	...
	st01 = v2UVcoords[0] - v2UVcoords[1];
	st02 = v2UVcoords[0] - v2UVcoords[2];
